...
local_sdk_version=$(cat $root/api_version)
...
result=$(curl -s -H "Authorization: token $GH_TOKEN" "https://api.github.com/repos/outscale/osc-api-deploy/pulls" | jq ".[] | select(.title == \"SDK v$new_sdk_version\") | .title")
